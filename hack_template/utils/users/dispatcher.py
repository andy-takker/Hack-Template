...
from hack_template.utils.auth.base import IAuthProvider
from hack_template.utils.auth.models import AuthTokenModel, AuthUser
from hack_template.utils.auth.passgen import Passgen
from hack_template.utils.exceptions import UserWithUsernameAlreadyExistsException
from hack_template.utils.users.models import CreateUserModel, LoginUserModel
...
class UserDispatcher:
    ...
    async def create(self, new_user: CreateUserModel) -> AuthTokenModel:
        if await self.user_storage.read_by_username(username=new_user.username):
